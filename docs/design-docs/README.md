...
- [Command Suggest](command-suggest.md) — Commands pane `+` 菜单列出从项目清单（package.json / Makefile / justfile / Taskfile / mise / Cargo …）识别的命令，一键加入；`CommandSuggestionParser` 纯解析器 + registry 扩展点，本地 / SSH 共用一次读取
