...
- 无 stdout 正则扫描（见 Alternatives A1）。未产生这些信号的工具没有对应的等待输入 / 命令完成通知；Pane 退出与崩溃仍按生命周期事件处理。
...
2. **检测与策略分离。** 事件分类、catalog 上下文解析与通知投递由三个组件承担：
   - `PaneAttentionInterpreter`——**纯函数**；`(event, context) → Step`。它以输入的形式生长旋钮，绝不以状态的形式。
   - `NotificationDetector`——编排：catalog 遍历、muted 标签丢弃、`hasProducedOutput`、击键上下文；产出一个 `Candidate`。
   - `NotificationCoordinator`——**策略闸**：读取实时设置 + 授权状态，向各副作用 sink 派发。投递设置与授权判定集中在这里；来源 Pane 的静音标签由检测器处理。

```text
TerminalEvent + keyboard timestamps + notification settings
                         |
                         v
              NotificationDetector
         catalog path / mute label / context
                         |
                         v
          PaneAttentionInterpreter.interpret
                         |
                      Candidate
                         |
                         v
             NotificationCoordinator
             focus / settings / authorization
                         |
          +--------------+--------------+
          v              v              v
   NotificationStore  OSNotifier    DockBadger
          |
          v
   unread 0 -> N edge -> HierarchyClient.promoteWorktree
```
...
- **检测与策略必须分离。** 事件翻译和呈现策略由不同层承担：纯翻译器 `PaneAttentionInterpreter`、编排器 `NotificationDetector`、唯一策略闸 `NotificationCoordinator`。

代价是覆盖缺口：既不发 OSC 9、也不响铃、也不发 OSC 133 的工具不被覆盖——这是被记录的限制（见「非目标」与「风险」），该边界由工具是否发出可识别事件决定。

## 检测（`PaneAttentionInterpreter`，纯函数）

`interpret(_ event: TerminalEvent, context: Context) -> Step` 是纯函数；`Context` 携带 `hasProducedOutput`、`lastUserKeystrokeAt: [PaneID: Date]`、注入的 `now`，以及命令结束相关设置。翻译表：
...
| `paneIdle(duration)` | "task idle for …" | `.taskFinished`——仅当 `duration ≥ 30s` 且 pane 近期有输出 |
...
4. 事件前 **3 秒**内来源 pane 有击键 → 丢弃（`userTypingRecently`）——用户显然正盯着这个 pane。
5. 否则发出，且**非零退出码给出一眼可辨的标题**（"Command failed (exit N)" vs "Command finished"）。

`Step` 带一个可选 `drop: InboxDropReason?`，使抑制与 coordinator 后续的丢弃走同一条日志路径。`InboxDropReason` 放在 `CodansCore`，让纯翻译器和 app 层 coordinator 共享同一个字符串编码枚举。

**击键旁路。** `PaneKeyboardActivityTracker`（`@MainActor`，持有 `[PaneID: Date]`）在终端键盘输入时记录，pane 拆除时清理。为什么不用 `TerminalEvent.paneUserInput` 事件：击键节奏是人类尺度（约 10 Hz），且只有检测器关心——新增事件 case 会给每个 `TerminalEvent` 消费者（`RootFeature`、测试、分析）添噪。为什么不用 `Pane.labels` / `@Observable` 字段：labels 会被持久化（每次击键一次 catalog 保存），`@Observable` 字段会让 SwiftUI 每次击键重渲染，而 `GhosttySurfaceView` 刻意避免了这一点。
...
3. `systemEnabled && authStatus == .authorized` → `OSNotifier.post(entry, playSound: soundEnabled)`；`.notDetermined` 和 `.denied` 均不投递，也不在此请求授权。
4. `moveNotifiedWorktreeToTop` 且该 worktree 的未读刚从 `0 → N` → `reorderWorktrees`。

coordinator 在追加前后读取 inbox 中该 Worktree 的真实未读数，只有未读数增加且追加前为 0 才触发提升。去重合并、容量淘汰和已读条目不会被误算成新的 `0 → N` 边沿；没有独立的 `unreadByWorktree` 缓存。
...
`Settings.notifications` 是通知设置 section。字段通过 `decodeIfPresent` 解码，文件中缺失的字段采用默认值。投递字段为 `inAppEnabled`、`systemEnabled`、`soundEnabled`、`dockBadgeEnabled`；指示器字段为 `statusBarBellEnabled`、`projectBellEnabled`、`worktreeBellEnabled`、`tabBellEnabled`，只控制显示、不影响收集。`moveNotifiedWorktreeToTop` 控制提升；`commandFinishedEnabled` 与 `commandFinishedThresholdSec` 控制命令完成检测（阈值默认 10 秒，解码与 UI 钳到 `[1,3600]`）。以上布尔值默认开启。`mute` 保存 `mutedRuleIDs` / `mutedPaneIDs` 集合，Settings 仅显示数量；检测与投递路径不读取这些集合。逐 Pane 静音的运行时入口是 `Pane.labels`。

**为何是四个正交布尔而非一个 `level` 枚举**：用户需要枚举表达不出的交叉组合（in-app 关 + system 开 =「仅后台」）。Sound 与 Dock badge 又是另外两个独立维度。
...
系统通知授权由用户在 **Settings → Notifications** 显式操作：System notifications 开启且状态为 `.notDetermined` 时显示 **Request…**，点击后调用 `OSNotifier.requestAuthorization()`；状态为 `.denied` 时显示 **Open System Settings…**。启动和首条通知均不会自动发起授权请求。

`NotificationCoordinator` 只把已授权的候选交给 `OSNotifier.post`；因此 adapter 内部的 `.notDetermined` 请求分支不是生产通知链路的首次授权入口。授权状态在启动和 `applicationDidBecomeActive` 时重读。OS 授权只约束系统横幅与音效；inbox、应用内徽标和 Dock 徽标仍分别受焦点策略、`inAppEnabled`、`dockBadgeEnabled` 等设置约束。

## 组件边界

```
CodansCore/
  Notifications/{InboxEntry, InboxStorage, PaneAttentionInterpreter(+Context),
                 InboxFile, RollupIndex, InboxDropReason}.swift
...
依赖方向：`InboxEntry ← PaneAttentionInterpreter/Detector → Coordinator → {Store, OSNotifier, DockBadger, HierarchyClient}`；各表面观察 `RollupIndex`。store 在设计上对 UI 与 OS 一无所知；`Notifications/` 除设置面板与 pane 上下文菜单外不 import 任何 UI 类型。

## 备选方案（Alternatives）

- **A1 — stdout 正则扫描器。** 否决：模式会漂移（"(y/n)" 会命中聊天记录），而一旦发一套正则就会招来规则编辑器。记录「请在你的工具里发 OSC 9」比永久维护这套正则更便宜；若真有用户撞上缺口，扫描器可作为增量回归。（根因见「技术决策」。）
- **A2 — 侧栏 inbox 路由。** 否决：通知本质是瞬态的（读 → 点进 → 忘掉），常驻路由把它们过度提升。铃铛 + popover 才贴合实际流程。
...
| 用户 shell 无 OSC 133——无 `commandFinished`。 | `paneExited` 覆盖 Pane 子进程退出，不代表其中每个前台命令结束。 |
| 创建与点击之间 catalog 变更。 | 原始 ID 存储 + 死目标回退（G3）；行保留并标记。 |
| 授权未请求或被拒。 | 系统横幅不投递；Settings 提供 Request / Open System Settings。inbox 与徽标按各自设置工作。 |
| 3 秒击键窗口与 IME 批量提交。 | tracker 在每次 `sendKey`（含合成事件）记录；若真有 IME 案例，把该常量提升为设置即可。 |
| `reorderWorktrees` 与手动拖拽竞争。 | 两者均 `@MainActor`，串行化；最后写入胜出——自动提升输给进行中的手动拖拽是正确结果。 |
| 降级时前向版本 inbox 被隔离。 | 隔离是重命名而非删除，可恢复；隔离路径由 `loadedQuarantineBackupURL` 暴露；重命名失败仍以空 inbox 启动，因此不能保证原文件一定已备份。 |
