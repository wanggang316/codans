...
App/Features/CreateWorkspace/
  ...
  CreateWorkspaceSheet        分组 Form 外壳（工作区一节 + 项目列表）+ presenter
  WorkspaceMemberRow          列表行：来源图标、名称与来源、检出摘要、问题 / 进度、编辑与删除
  WorkspaceMemberEditorSheet  Add Project / 编辑弹窗（本地 / 远程两种来源 + 检出设置）
...
- GUI（`App/Features/CreateWorkspace/`）：`CreateWorkspaceFeature` + `CreateWorkspaceSheet`（Add 菜单「New Workspace…」、palette `app.new-workspace`、`CommandID.newWorkspace` 默认无绑定；workspace 根行的 `+`「Add Repository…」打开同一 sheet 的 add 模式：Workspace / Location 只读、恰好一个成员、提交走 `addStream`）。样式与 Settings 面板一致：`Form` + `.formStyle(.grouped)`，行用 `TextField` / `LabeledContent` / `Picker`（原生弹出菜单），不自绘控件；Cancel / Create 放在表单下方的按钮栏（不叠在表单上，避免行滚到按钮后面）；宽 560，高度随内容，最高 760 后滚动。
  - 第一节「New Workspace」：Title；Location（父目录 + 文件夹按钮，显示 `<location>/<slug>`，文件夹名始终跟随标题）；最后一行是「Add Project」下拉（Settings 面板「Add …」行的样式），两项 Local Repository… / Remote Repository… 各自打开一个弹窗。没有统一的分支设置：New branch 留空的分支名取标题 slug（`State.defaultBranch`），填了就用自己的。当前侧栏显示为选中的 Project（`Catalog.displayedSelectedProjectID`，与选中流同一回退规则）打开即在列表里。
  - 第二节「Projects」：每个项目一行——来源图标（已注册 Project 用它自己的 `ProjectIconView`，磁盘上的文件夹 `folder`，远程 `globe`）、文件夹名 + 来源路径或 URL（中间截断，悬停看全文与 clone 目标）、一行检出摘要（`checkoutSummary`：New branch X from Y / Branch X / Branch X, tracking origin/X …）、红 / 橙问题行；右侧铅笔（编辑）与垃圾桶（删除）。创建中右侧换成进度圈 / 勾，摘要下显示阶段与最后一行进度、失败原因或 Rolled back。空列表显示一行说明。
  - 弹窗（`WorkspaceMemberEditorSheet`，`State.editor: MemberEditor`，同为分组 Form，宽 500，底部 Cancel / Add 或 Save）：第一节是来源——本地为 Repository 弹出菜单（未在列表中的已注册 Project、上次选的文件夹、分隔线、Other Folder… 打开文件夹选择器；`inspectRepository` 后命中已注册 Project 记为 Project，裸仓库与非仓库给出原因，按 canonical 根去重）；远程为 URL 输入（停顿 600 ms 或 ⏎ 后生效；`AddEntryClassifier` 识别，路径与无法识别的文字只在 ⏎ 时报错；按 host/path 去重）+ Clone to（默认 `~/.codans/sources/<name>`，选过的父目录在换 URL 后保留）；编辑时只读显示 Repository（名称 + 路径或 URL），远程仍可改 Clone to。选定来源后出现第二节：Checkout（New branch / Existing branch / Remote branch，未 clone 的远程源不提供 Existing branch）、该模式的分支行、Folder name（默认仓库名，与列表或 add 模式已有名字冲突时加 `-2`；手改后换来源不再改名）。New branch 的 Branch 占位符显示标题 slug，Based on 首项为「Default (origin/main)」；Existing branch 只列本地分支，被其他 worktree 检出的标注 checked out；Remote branch 不预选，选中的远程分支在本地已存在时出现 Local branch 选择（Keep local branch 默认 / Reset to <ref>）。节脚显示分支加载状态与 Retry 和问题。
