...
- **Cipher suite.** TLS 1.2 with a PSK suite, preferring `TLS_ECDHE_PSK_WITH_AES_128_GCM_SHA256` for forward secrecy. If the SDK's `tls_ciphersuite_t` does not expose that suite, fall back to `TLS_PSK_WITH_AES_128_GCM_SHA256` (the suite Apple's own peer-to-peer samples use) and record the choice in the decisions table. TLS 1.3 PSK is not used because Network.framework's external-PSK API is exposed for the 1.2 suites.
...
- **Server side.** `sec_protocol_options_set_pre_shared_key_selection_block` receives the client's PSK identity hint, looks the device up in `PairedDeviceStore`, and completes with that device's key from the Keychain — or with nothing, which fails the handshake. The identity is the device ID; the resolved device ID is then bound to the connection as its `CallerContext`.
...
- **Proof first.** The first M1 task is a loopback unit test (`NWListener` + `NWConnection` in-process) that proves: correct key handshakes, wrong key is rejected, unknown identity is rejected, revoked identity is rejected. If PSK handshakes are not viable on the installed SDK the design falls back to a self-signed certificate pinned in the pairing payload (see Alternatives).
...
| `.interactive` | `terminal.sendInput`, `terminal.sendKey`, `terminal.retryPane`; `agent.launch`; `hierarchy.activateWorktree`, `hierarchy.activateTab`, `hierarchy.focusPane`, `hierarchy.createTab`, `hierarchy.openPane`, `hierarchy.splitPane`, `hierarchy.zoomPane`, `hierarchy.unzoomPane` |
...
- **Features** (TCA reducers end in `Feature`, views in `View`): `Connection` (browse, pair, reconnect), `Agents` (event-driven list grouped as *needs input* / *working* / *idle*, tapping opens the pane), `Browser` (Project → Worktree → Tab → Pane), `PaneDetail` (text snapshot, input bar, key row Enter / Esc / Ctrl-C / Tab / ↑ / ↓, hidden for read-only devices; iPad hardware-keyboard shortcuts via `.keyboardShortcut`), `Settings` (paired Macs, forget).
...
2. **Standard containers.** Root `TabView` (Agents / Browse / Settings) with `.tabViewStyle(.sidebarAdaptable)`; Browse is a three-column `NavigationSplitView`; actions live in `.toolbar`. From iOS 27.1 standard navigation bars and toolbars move to the side on the Duo inner display automatically; custom bars would need the 27.1 `ReservedRegion` API, which v1 does not use because it draws no custom bars.
...
| D4 | TLS-PSK with a random 32-byte key per device, selected by PSK identity on the server; `ECDHE_PSK` suite preferred, `PSK_WITH_AES_128_GCM_SHA256` if the SDK lacks it. | Mutual auth without certificates; per-device revocation. |
