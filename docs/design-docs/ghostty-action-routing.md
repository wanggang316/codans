...
路由 codans 有明确映射的每一个 ghostty action，按四个分发语义归桶。每个 case 要么同步解码为宿主支持的动作（返回 `true`，随后直接或异步 apply），要么解码为带日志的 `.unsupported`（返回 `false`）。支持的 intent 提升到 `TerminalEvent` 流，由 TCA feature 消费。**没有静默 fall-through。**
...
- **Search overlay UI、secure-input 视觉指示、scroll bar 渲染、progress bar overlay。** 这些归后续 UI feature。本设计只把状态*存*在 `SurfaceInfo` 上并发 `paneInfoChanged`，供未来 overlay 消费。
- **多窗口模型重设计。** window intent 桶交给当前主窗口的接收方处理；本文定义 intent，接收方决定策略。
...
- 一个 action 对应一个 case；
- intent case 发 `emit(.paneActionRequested(…))` 或 `emit(.windowActionRequested(…))`；
- info case 写 `pane.apply(delta)` + `emitPaneInfoChanged`；
- effect case 跑副作用 + 发可观测事件；
- 不支持的 case 记日志并返回 `false`；
- `paneActionRequested` / `windowActionRequested` 各自的 router 是这些事件的**唯一订阅者**（架构不变量，code review 把关，防双重消费）。
...
C thunk 在同步解码后取 `decoded.consumed`：`.unsupported` 为 `false`，其余解码结果为 `true`。主线程直接执行 apply；非主线程将 Swift-owned payload 排入 `DispatchQueue.main.async` 后立即返回**同一个 consumed 值**。该值表示 action 已由宿主接管，不表示异步副作用已经完成；不能因等待主线程而一律返回 `false`。
...
### `TerminalEvent` case 与 Core 类型

`CodansCore/TerminalEvent.swift` 定义 `.paneInfoChanged(PaneID, PaneInfoDelta)`、`.paneActionRequested(PaneID, PaneActionRequest)`、`.windowActionRequested(WindowActionRequest)`、`.configChanged`。

`CodansCore/` 定义三个 `Sendable, Equatable` 枚举：`PaneInfoDelta`（title / pwd / mouse / geometry / scrollbar / secureInput / search / progress / bell / commandFinished / childExited 等的逐字段 delta）、`PaneActionRequest`（tab/split intent）、`WindowActionRequest`（window/app intent）。ghostty 的 C enum 类型留在解码器内，不进 Core。
...
- **`PaneActionRouterFeature`** 订阅 `paneActionRequested`，把 `PaneID` 解析为 `(ProjectID, WorktreeID, TabID)`（经 `HierarchyClient.addressOf(paneID:)`），再分发到 `HierarchyClient`（createTab / closeTab / moveTab / activateTab / splitPanel / focusPanel / resizePanel / equalizeTabSplits / zoomPanel）与 `UIClient`（presentTerminal / toggleCommandPalette）。
...
终端右键菜单与 Pane HUD 的终端操作共享 `PaneSurfaceAction.groups`，包括复制、粘贴、四向拆分、重置终端、复制 Pane ID 与关闭 Pane。没有选区时，右键菜单隐藏复制项，HUD 保留禁用项。交接、命令队列和通知静音属于 HUD 自身的层级操作，不在这份 surface 操作列表中。

Source: [PaneSurfaceAction.swift](../../apps/mac/codans/Runtime/Ghostty/PaneSurfaceAction.swift), [GhosttySurfaceView.swift](../../apps/mac/codans/Runtime/Ghostty/GhosttySurfaceView.swift), [PaneHUDView.swift](../../apps/mac/codans/App/Features/PaneHUD/PaneHUDView.swift).
...
**可观测性。** `GhosttyActionDecoder` 通过运行时日志记录 action 处理与未支持 tag。`system.status` 不暴露逐 tag 计数；没有 `unhandledActionCounts` 或 `CODANS_DISABLE_ACTION_ROUTING` 配置入口。
...
| 风险 | 缓解 |
|---|---|
| Action 洪流压垮扇出（chatty TUI 每个 prompt 都发 `SET_TITLE`/`PWD`/`PROGRESS_REPORT`）。 | `TerminalEvent` 上 `.bufferingNewest(256)`；仅当字段真变化才发 `paneInfoChanged`（`apply` 做 diff memo）；生命周期事件不缓冲。 |
| 线程安全：回调在非主线程触发，userdata 与 `PaneSurface` deinit 竞态。 | 两遍解码在任何主线程跳转前就把 16 字节 + 指针字段复制为 Sendable 值；注册表访问 `@MainActor`。 |
| 未支持的 action tag。 | 解码为 `.unsupported`，日志保留 tag 与原因，C 回调返回 `false`。 |
