...
**Author:** Gump
**Date:** 2026-05-24
**Spec:** docs/product-specs/pane-resume.md (设计文档已下线)
...
**Outcome:** After quitting codans while a `tail -f` is running and then relaunching, the same shell process is still attached to the same pane and is still streaming output that was produced while the app was closed.
...
8. Relaunch the app via `open -a Codans`, wait for ready signal
9. Wait until the pane re-attaches (ready signal "Pane attached")
10. Capture content: `codans pane read <paneID> --tail 20 --json | jq -r '.content' > /tmp/pane-content-after.txt`
...
- `ps-zmx.txt` from `ps -eo pid,ppid,sid,command | grep -E '(zmx|Codans)' > ps-zmx.txt`
- `runtime.log` from `log stream --predicate 'subsystem == "com.gumpw.codans.runtime"' --last 5m`
...
**Steps:**
1. In the open pane, run: `codans pane send <paneID> $'echo "first line"; echo -e "\\033[33mSECOND-LINE\\033[0m"\r'`
2. Wait until `codans pane read <paneID> --tail 4` includes both lines
3. Capture the full pane buffer: `BUFFER_PRE=$(codans pane read <paneID> --raw)` (raw VT, no normalization)
4. Capture shell PID: `PID_PRE=$(codans pane info <paneID> --json | jq -r '.shellPid')`
5. Quit the app via the Quit menu item
6. Wait until `pgrep -x Codans` returns empty
7. Wait until the snapshot file appears: `~/Library/Caches/codans/snapshots/<paneID>.snap` (poll up to 5s)
8. Confirm the daemon's PID `$PID_PRE` is gone: `kill -0 "$PID_PRE"` returns nonzero
9. Relaunch the app via `open -a Codans`, wait for ready signal and "Pane attached"
10. Capture the new buffer: `BUFFER_POST=$(codans pane read <paneID> --raw)`
11. Capture the new shell PID: `PID_POST=$(codans pane info <paneID> --json | jq -r '.shellPid')`

**Assertions:**
1. (File) After step 7, the snapshot file exists and is non-empty (`test -s ~/Library/Caches/codans/snapshots/<paneID>.snap`)
...
**Steps:**
1. Produce 200 lines of distinctive output: `codans pane send <paneID> $'for i in $(seq 1 200); do printf "line-%03d\\n" "$i"; done\r'`
2. Wait until `codans pane read <paneID> --tail 1` contains `line-200`
3. Move cursor mid-line and emit a partial-line marker: `codans pane send <paneID> $'printf "PARTIAL"; printf "\\r"; printf "X"\r'`
4. Capture buffer + cursor: `codans pane info <paneID> --json > /tmp/pane-info-pre.json`; `codans pane read <paneID> --raw --full > /tmp/buffer-pre.txt`
5. cmd-Q the app, wait until process is gone
6. Relaunch, wait for "Pane attached"
7. Capture again: `codans pane info <paneID> --json > /tmp/pane-info-post.json`; `codans pane read <paneID> --raw --full > /tmp/buffer-post.txt`
...
4. (CLI) `codans pane read <paneID> --range scrollback --tail 1` contains `line-001` (the oldest line scrolled out is still in scrollback)
...
**Steps:**
1. Enable application keypad mode and an alt screen marker: `codans pane send <paneID> $'printf "\\033[?1049h"; printf "ALT-SCREEN-MARKER\\n"\r'` (enter alt screen, write marker)
2. Set a non-trivial cwd: `codans pane send <paneID> $'cd /tmp\r'` (shell integration emits OSC 7)
3. Wait until `codans pane info <paneID> --json | jq -r '.pwd'` equals `/tmp`
4. cmd-Q app, wait until gone
5. Relaunch, wait for "Pane attached"
6. Capture state: `codans pane info <paneID> --json > /tmp/info-post.json`; `codans pane read <paneID> --raw --range visible > /tmp/visible-post.txt`
...
#### Case `UT-PANE-RESUME-005`: `codans pane close` removes daemon, socket, and catalog entry within 2s
...
**Steps:**
1. Launch the app: `open -a Codans`, wait for ready signal
2. Wait until `log stream --predicate 'subsystem == "com.gumpw.codans.runtime"' --info` emits a line containing `reaper killed session=STALE-PANE-ID` (or the chosen log marker)
3. Activate the stale pane via the UI (or `codans pane focus STALE-PANE-ID`)
4. Wait for "Pane attached"
5. Capture new state: `codans pane info STALE-PANE-ID --json > /tmp/info-after-reap.json`
...
**Persona:** `dev_running_long_task`
**Outcome:** A second `Codans.app` process launched while the first is running cannot attach to the first instance's daemons; the second instance either declines its panes or cold-starts them. No daemon serves two clients in v1.
...
**Steps:**
1. Launch a second instance: `open -n -a Codans` (the `-n` forces a new instance)
2. Wait until two `Codans` processes exist: `pgrep -x Codans | wc -l` returns `2`
3. Capture the second instance's view of the pane: in the second instance, query `codans pane info <paneID> --json --instance second > /tmp/second-info.json` (the `--instance` flag is a precondition for this case; the runner is responsible for routing to the right socket)
4. Capture the first instance's pane: `codans pane info <paneID> --json > /tmp/first-info.json`
...
- `ps-codans.txt` from `ps -eo pid,ppid,command | grep Codans`
...
**Preconditions:**
- App started; `resumePanesOnLaunch=true`
- Single-pane fixture; pane attached; the shell is busy: `codans pane send <paneID> $'while true; do echo tick; sleep 1; done\r'`
...
5. Relaunch: `open -a Codans`, wait for ready signal
6. Wait for "Pane attached"
7. Capture: `codans pane info <paneID> --json | jq -r '.shellPid'` and `codans pane read <paneID> --tail 3`
...
4. (CLI) `codans pane read <paneID> --tail 3` shows more `tick` lines than at step 1 (output continued accumulating while app was dead)
...
4. **Does `codans pane read` need a `--raw` option to expose VT escape sequences un-normalized for byte-equality assertions (UT-PANE-RESUME-003 assertion 3)?** *Default:* yes — this is a precondition for the case to pass. Surface to planner as a codans CLI addition.

---

**Downstream:**
- `/hs-planner` binds tasks to these case IDs (UT-PANE-RESUME-001…008) and adds `codans pane read --raw` / `codans pane close` / `codans pane info --json` / `--instance` to the CLI surface as task dependencies.
