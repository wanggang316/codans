...
| C10 | Read-only diff viewer | Changes shows aggregate current changes, including untracked files; Outgoing compares the merge base of the selected or PR target branch to HEAD. Includes unified / split rendering, syntax highlighting, a native tree/list file sidebar with file-type icons, file filtering, and current-file editor handoff. See [Git diff viewer](design-docs/git-diff-viewer.md) | Implemented | Experimental |
...
- Built-in read-only Changes / Outgoing window, opened from the toolbar, menu, or command palette. It retains per-worktree scope, base, and file selection for the app session and refreshes local Git state every two seconds while visible; it does not fetch remote refs automatically
