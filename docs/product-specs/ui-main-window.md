...
codans 的主窗口是一个两栏布局：左侧 **Sidebar** 列出全部 Project 及其 Worktree，底部钉一条 **Tag filter** chip 条；右侧 **Detail** 区在终端之上有一行 **Header**，显示当前分支与一个 "Open in …" split button。终端本体（Tab 条 + 分屏 Pane）在 Header 之下。

本规格描述可观测行为与需求。它定义：侧栏如何呈现 Project / Worktree、Tag filter 如何工作、Header 承载什么、未读状态在 UI 何处出现，以及用户可控的策略。实现选择见 [main-window 设计文档](../design-docs/main-window.md)。
...
Legend: `●` active Worktree, `○` inactive Worktree, `[+]` hover-only add, `[⋯]` options menu。Tag filter chip 条在侧栏底部，溢出时横向滚动。

注：通知铃铛**不在** Header 上。未读通知通过状态栏铃铛 + popover 呈现（见 [notifications.md](notifications.md)）；Git diff 通过 ⌘⇧G / 命令面板 "Toggle Git Viewer" 打开的 **Diff inspector** 呈现（见 [diff-inspector.md](../design-docs/diff-inspector.md)）。两者都不挂在主窗口 Header 上。

## User Stories

- 作为多 Project 开发者，我想在一棵树里看到全部 Project 与 Worktree，从而无需翻菜单就能切 Worktree。
- 作为给 Project 打多重标签的用户（`#client-acme` / `#urgent` / `#archive`），我想要侧栏底部的一条 Tag filter，从而把视图收窄到我关心的那一组 Project。
...
│                          │
├──────────────────────────┤
│ [All] [● acme] [Untagged]│ ← Tag filter footer（always visible）
└──────────────────────────┘
```
...
底部的 **Tag filter footer** 取代了早先的 Space switcher：
...
- 每个 Tag 一个 chip，外加隐含的 `[All]` 与 `[Untagged]`。点击 chip 切换它在 `Catalog.activeTagFilter` 中的成员资格（多选 OR）。
- `[All]` 清除 filter；`[Untagged]` 互斥（选它会取消所有 Tag chip），且仅当至少一个 Project 无标签时才出现。
- footer 在底部，匹配 macOS 侧栏惯例（System Settings、Mail），并让顶边保持干净。
...
Header 现在只承载两样：左侧只读分支标签，右侧 "Open in …" split button。它**不**承载通知铃铛，也**不**承载 Git Viewer 切换按钮——这两个能力随产品演进迁出了 Header：

- **通知**：状态栏铃铛是唯一的 inbox popover 入口；未读以按层级上卷的徽标呈现（见下文 Display 与 [notifications.md](notifications.md)）。
- **Git diff**：通过 ⌘⇧G、Header 之外的命令面板 "Toggle Git Viewer"、或菜单触发 **Diff inspector**（右缘列出改动文件 + 全区 diff drawer），可见性按 Worktree 持久化（见 [diff-inspector.md](../design-docs/diff-inspector.md)）。
...
- [x] Tag filter footer 钉在 Sidebar 底部、始终可见。每个 Tag 一个 chip，外加隐含的 `[All]` 与 `[Untagged]`。点击 chip 切换其在 `Catalog.activeTagFilter` 中的成员（多选 OR）；`[All]` 清 filter；`[Untagged]` 互斥且仅在存在无标签 Project 时出现。
...
- [x] 一个键盘快捷键聚焦 Tag filter（⌘F）；输入即就地过滤 chip，Return 提交，Esc 清回 `.all`。
...
- [x] Sidebar 顶部对大列表的过滤——以 Tag filter 形式实现（按 Tag 收窄，而非自由文本搜索框）。

### Won't Have

- 从 Header 编辑分支（rename / checkout / switch）。分支标签只读。（分支切换器是独立能力，见 [worktree-management.md](worktree-management.md)。）
- 常驻第三列的 inspector。Diff inspector 以右缘 + drawer 呈现，不是常驻列。
...
- 通知铃铛 / Git Viewer 切换按钮挂在 Header 上——二者已分别迁到状态栏 popover 与 ⌘⇧G/命令面板。

## Acceptance Criteria

- 给定活跃 Tag filter 为 `.all`，当用户点击某个 Tag chip，则侧栏只显示带该 Tag 的 Project；再点 `[All]` 恢复全部。
- 给定多个 Project 有不同 Tag，当用户选中两个 Tag chip，则侧栏显示带其中任一 Tag 的 Project（OR 语义）。
...
- 给定 Diff inspector 关闭，当用户按 ⌘⇧G（或命令面板 "Toggle Git Viewer"），则右缘 Diff inspector 出现并可见地列出改动文件；再触发隐藏。可见性按 Worktree 持久化（`Worktree.diffInspectorVisible`）。

## 设计沿革（History）

本窗口最初以一次"主窗口重构（redesign）"立项，分四个子任务（T0 契约 / T1 侧栏 / T2 Header / T3 Git Viewer overlay）推进。其后两次产品演进改变了几处现状，本规格已按现状陈述：

- **Space → Tag**（[project-tags.md](../design-docs/project-tags.md)）：层级从 5 级降到 4 级；侧栏底部从 Space switcher 改为 Tag filter；⌘1–⌘9 / ⌘K 的 Space 跳转解绑；per-Space "上次活跃 Worktree" 记忆移除（per-Project `selectedWorktreeID` 已覆盖）。
- **Git Viewer → Diff inspector**（[diff-inspector.md](../design-docs/diff-inspector.md)）：Header 上的 Git Viewer 切换按钮移除；改由 ⌘⇧G / 命令面板 / 菜单触发右缘 inspector + drawer；`Worktree.gitViewerVisible` 重命名为 `Worktree.diffInspectorVisible`。
- **通知铃铛迁出 Header**（[notifications.md](notifications.md)）：未读以按层级上卷的徽标呈现，唯一 popover 入口是状态栏铃铛。
...
- Diff inspector：[diff-inspector.md](../design-docs/diff-inspector.md)
