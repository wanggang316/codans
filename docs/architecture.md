...
- `scripts/xcode-compat/` lets the pinned zig 0.15.2 build on Xcode 26.5+: `build-ghostty.sh` / `build-zmx.sh` source its `activate.sh`, which probes the SDK and libtool and puts `xcrun` / `libtool` wrappers on PATH only when needed (see `docs/lessons-learned/2026-09-17-zig-builds-broke-on-xcode-26-5-and-26-6.md`)
