...
- **TestStore effects that reach live-instance dependencies spawn real subprocesses and miss receive windows under parallel-suite load.** `WorktreeLocalDiffMonitor.testValue == liveValue`, so any un-overridden reducer path through it runs a real `git diff` — under load that blows TCA's 1.0 s default `receive` window (seen as `Expected to receive a matching action, but received none after 1.0 seconds`, passing in isolation, failing in a full run). Fix by injecting a no-op instance (`WorktreeLocalDiffMonitor(fetch: { _ in nil })`), not by raising timeouts — 5 s was observed to still lose under load.
