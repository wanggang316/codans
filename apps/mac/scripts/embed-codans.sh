...
# build driver could target unintended paths. Hard-fail instead.
...
# This directory is shared with zmx. Incremental builds may skip its embed
# phase, so this phase must replace only its own declared output.
