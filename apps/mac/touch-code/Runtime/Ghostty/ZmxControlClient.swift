...
nonisolated enum ZmxControlClient {
  private static let logger = Logger(
    subsystem: "com.touch-code.runtime", category: "runtime.zmx.control"
  )
  ...
  /// binds `<ZMX_DIR>/<session>` (touch-code sets no `ZMX_SESSION_PREFIX`).
  ...
  /// `tc pane read`). Returns the raw `.history` payload bytes.
  static func history(
