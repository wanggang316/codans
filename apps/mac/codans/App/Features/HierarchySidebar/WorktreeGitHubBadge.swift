...
struct WorktreeGitHubBadge<PopoverContent: View>: View {
  ...
  var body: some View {
    ...
    Group {
      if let snapshot {
        // `checkRollup` travels on the snapshot (populated by the batched
        // `gh api graphql` path), not the retired `state.checks[prNumber]`
        // dictionary. The `+N −M` chip lives in
        // `HierarchySidebarView.diffStatsChip` so PR-less rows can surface
        // the same widget against `WorktreeBranchDiffMonitor`.
