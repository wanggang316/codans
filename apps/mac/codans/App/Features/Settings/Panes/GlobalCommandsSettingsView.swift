...
/// Global Commands sub-pane — the user-defined `GeneralSettings.globalScripts`
/// rendered with the same inline, spreadsheet-style `ScriptCommandTable` the
/// per-Project Commands pane uses. Unlike the Project pane there is no built-in
/// Run invariant: the global list starts empty and every row is freely
/// addable / removable.
///
/// Reads come from `@Environment(SettingsStore.self)` for live updates; writes
/// always go through the TCA reducer so test stores can spy on individual
/// writes without instantiating the SwiftUI view.
struct GlobalCommandsSettingsView: View {
  @Bindable var store: StoreOf<GlobalCommandsFeature>

  @Environment(SettingsStore.self) private var settingsStore
  /// Resolved system-shortcut map. Drives chord-conflict detection against
  /// every registered CommandID at recording time.
  @Environment(\.resolvedShortcuts) private var resolvedShortcuts: ResolvedShortcutMap

  /// Currently selected command row in the inline table. Drives the row
  /// highlight and the `−` (remove) button. Lifted out of the table so
  /// `addScript()` can select the freshly-created row.
  @State private var selectedScriptID: UUID?

  // MARK: - Derived state

  private var scripts: [ScriptDefinition] {
    settingsStore.settings.general.globalScripts
  }
  ...
  /// Reject reserved / conflicting chords at recording time. Mirrors the
  /// Project Commands pane's order: macOS system reservations, then AppKit
  /// standard menus, then the app's own registered CommandIDs, then sibling
  /// global commands.
  ...
  /// unique-ish placeholder name; predefined kinds fall back to their kind name
  /// (`displayName`) so the row reads "Run", "Test", etc. until renamed.
  private func addScript(kind: ScriptKind) {
    ...
    store.send(.setGlobalScripts(scripts + [new]))
  ...
  private func moveScript(id: UUID, offset: Int) {
    ...
    store.send(.setGlobalScripts(updated))
  }
